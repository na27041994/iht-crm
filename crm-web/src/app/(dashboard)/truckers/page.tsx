...
export default function TruckersPage() {
  ...
  const canView = usePermission('trucker', 'view');
  const canCreate = usePermission('trucker', 'create');
  const canEdit = usePermission('trucker', 'edit');
  const canDelete = usePermission('trucker', 'delete');
  ...
  const load = useCallback(
    async (kw = '', pg = 1) => {
      ...
      try {
        ...
        const res = await apiFetch<ListResponse>(`/truckers?${params}`);
        setData(res);
      } catch (err) {
        message.error(err instanceof Error ? err.message : 'Không tải được danh sách nhà xe');
      } finally {
        setLoading(false);
      }
    },
    ...
    <div>
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <Typography.Text type="secondary">Danh sách nhà xe vận tải đường bộ</Typography.Text>
        </div>
        {canCreate && (
      ...
      {!canView ? (
        <Empty description="Bạn không có quyền xem nhà xe" />
      ) : (
        <>
          <Input.Search
          ...
          <Table<Trucker>
            ...
            scroll={{ x: 900 }}
            columns={[
              { title: 'Tên nhà xe', dataIndex: 'truckerName', render: (v: string) => <span className="font-medium">{v}</span> },
