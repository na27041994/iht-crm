...
export default function DebitReportPage() {
  ...
  const loadSheets = useCallback(
    async (pg = 1) => {
      ...
      try {
        ...
        const res = await apiFetch<{ items: any[]; total: number }>(`/reports/debit/sheets?${params}`);
        setSheets(res.items);
  ...
  function handlePrintSelected() {
    ...
    window.open(`/print/debit-notes?ids=${selectedSheetIds.join(',')}`, '_blank', 'noopener');
  }
  ...
  async function handleExportSelected() {
    ...
    setExporting(true);
    try {
      const blob = await apiDownload(`/reports/debit/export?ids=${selectedSheetIds.join(',')}`);
      saveBlob(blob, `debit_${selectedSheetIds.length}_phieu.xlsx`);
  ...
  async function handleExportAll() {
    ...
    try {
      ...
      const blob = await apiDownload(`/reports/debit/export${qs ? `?${qs}` : ''}`);
      saveBlob(blob, 'thong-ke-debit.xlsx');
      ...
      <Card>
        ...
        <Table
          ...
          columns={[
            ...
            { title: 'Số Debit', dataIndex: 'debitCount', width: 90, align: 'right' as const },
            { title: 'Tổng tiền Debit', dataIndex: 'debitTotal', width: 140, align: 'right' as const, render: fmtMoney },
          ]}
