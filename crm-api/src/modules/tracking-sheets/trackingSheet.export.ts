...
export async function buildTrackingSheetsWorkbook(sheets: TrackingSheetWithRelations[]): Promise<Buffer> {
  ...
  setupSheet(wsSheet, [
    ...
    { header: 'Container Qty', key: 'containerQuantity', width: 13 },
    ...
    { header: 'Số hóa đơn', key: 'invoiceNumber', width: 17 },
    { header: 'NV chứng từ', key: 'docStaff', width: 16 },
    { header: 'NV giao nhận', key: 'deliveryStaff', width: 16 },
    { header: 'Ghi chú', key: 'note', width: 26 },
  ...
  for (const s of sheets) {
    wsSheet.addRow({
      ...
      containerQuantity: s.containerQuantity ?? null,
      ...
      docStaff: s.docStaff?.fullName ?? '',
      deliveryStaff: s.deliveryStaff?.fullName ?? '',
      note: s.note ?? '',
...
export async function buildImportTemplateWorkbook(): Promise<Buffer> {
  ...
  const SHEET_COLS: Partial<ExcelJS.Column>[] = [
    ...
    { header: 'docStaffId', key: 'docStaffId', width: 12 },
    { header: 'deliveryStaffId', key: 'deliveryStaffId', width: 12 },
